...
from modules.mark_colon_lines import mark_lines_with_colon

#####---------------------------------------------------

# in scripts/parse_bohemia_listings_v1.py
...
def main(file, config_path, output_dir):
    ...
    if cfg.get("MakingStar") :
        filestar=load_lines(file)
        filestar = mark_lines_with_colon(list(filestar))
        # =====LOAD FILE AND PREPROCESS

        
        listings = preprocess_listings(filestar,
...
#==================
 ...
 # Ensure agency comes from args
    ...
    outdir = os.path.join(args.output_dir, agency, year)
    print("outdoe==>",outdir)
#=========
    if rows:
        ...
        outpath = outdir+"/"+agency.lower()+"_"+dateprint+".csv"
...
if __name__ == "__main__":
    ...
    print("[entry] starting parse_bohemia_listings_v2.py")
