...
# in scripts/parse_edc_listings_v1.py
...
# Agency comes from configuration file
    agency = cfg["agency"]

    # Derive date from filename
    file_name = os.path.basename(args.file)
    m = re.search(r'(\d{8})', file_name)  # e.g. 20151028
    date = m.group(1) if m else "unknown"

    # Extract year
    year = date[:4] if date and date != "unknown" else "unknown"

    # Build output directory: output/<agency>/<year>
    agency_folder = agency.lower()
...
#=========
    ...
    if rows:
        ...
        # Create full output path if it does not exist
        ...
        print(
            f"⚠️ No listings parsed. "
            f"Check header detection and marker in {args.file}."
        )
...
if __name__ == "__main__":
    ...
    print("[entry] starting parse_edc_listings_v1.py")
