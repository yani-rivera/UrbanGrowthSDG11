...
| `remove_words.txt` | TXT | Defines words removed from neighborhood strings to improve catalog matching |
...
**Purpose**  
Defines words or phrases that are removed from **neighborhood strings**
to improve matching against the Neighborhood Catalog.

**Why it exists**
- Neighborhood names in listings often include generic descriptors
  (e.g. "colonia", "barrio", "residencial")
- These words are not part of the canonical neighborhood name
- Removing them improves deterministic matching without guessing

**How it is used**
1. A neighborhood string is extracted from the listing
2. Words listed in `remove_words.txt` are stripped
3. The cleaned string is compared against catalog aliases

**Important**
- Removal happens **before** catalog matching
- The original extracted text is always preserved
