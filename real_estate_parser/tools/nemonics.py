...
def build_map(configs_dir: str, pattern: str, agency_keys, nemonic_keys, exclude_paths: Optional[set] = None) -> Dict[str, str]:
    paths = sorted(glob(os.path.join(configs_dir, pattern)))
    if exclude_paths:
        paths = [p for p in paths if os.path.abspath(p) not in exclude_paths]
    if not paths:
...
def merge_with_existing(out_path: str, new_map: Dict[str, str]) -> Tuple[Dict[str, str], bool]:
    ...
    try:
        ...
        existing_raw = existing_root.get("nemonics") or existing_root.get("mnemonics") or existing_root
        # Coerce mapping: handle cases where it's a stringified dict
        if isinstance(existing_raw, str):
            try:
                ...
                try:
                    ...
                    print("[WARN] Existing nemonics mapping is a string but not parseable; ignoring")
                    existing = {}
...
def run(args: argparse.Namespace) -> None:
    exclude = {os.path.abspath(args.output)}
    new_map = build_map(args.configs_dir, args.pattern, args.agency_keys, args.nemonic_keys, exclude_paths=exclude)
