...
def write_pre_file(records: List[str], agency: str, original_filename: str,
                   *, outdir: Optional[str] = None, outfile: Optional[str] = None) -> str:
    """Write the prefile to disk.

    If *outdir* or *outfile* are not provided, they are inferred as:
      outdir  = f"output/{agency}/pre"
      outfile = f"pre_{original_filename}"
    """
    # infer defaults
    if outdir is None:
        outdir = f"output/{agency}/pre"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--outdir", required=False, help="Output directory (default: output/<AGENCY>/pre)")
    ap.add_argument("--outfile", required=False, help="Output file name (default: pre_<original>.txt)")
    args = ap.parse_args()
    ...
    wrote = write_pre_file(records, agency=args.agency, original_filename=original_filename,
