...
def bulletize(lines: Iterable[str], cfg: dict | None = None, *,
              marker: str | None = None, to_marker: str | None = None,
              emit_marker: bool | None = None, drop_blank_lines: bool = True) -> List[str]:
    """Merge first, then standardize. Keeps headers intact.
    - If marker/to_marker/emit_marker are provided, they override cfg.
    - Returns list[str] where each listing is exactly one line.
    """
    in_mark, out_mark, emit = _read_markers(cfg, marker, to_marker)
    if marker is not None:
        in_mark = marker
    if to_marker is not None:
        out_mark = to_marker
    if emit_marker is not None:
        emit = bool(emit_marker)

    if not in_mark:
        raise ValueError("bulletize: missing input marker (set cfg['listing_marker'] or pass marker=)")

    merged = merge_listings(lines, in_mark, drop_blank_lines=drop_blank_lines)
    return standardize_marker(merged, from_marker=in_mark, to_marker=out_mark, emit_marker=emit)
